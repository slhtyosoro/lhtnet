...
class Instance extends EventEmitter {
    constructor(config, webConfig) {
        ...
        /* defaults */
        if (!config) {
            throw new Error('Instance requries a lhtnetConfig')
        } else {
    ...
    proxifyOrGetCachedProxy(tick, entity) {
        if (this.proxyCache[tick].entities[entity.id]) {
            ...
            if (!entity.protocol) {
                ...
                throw new Error('lhtnet encountered an entity without a protocol. Did you forget to attach a protocol to an entity or list it in the config? Did you add an entity to the instance that was never supposed to be networked?')
            }
